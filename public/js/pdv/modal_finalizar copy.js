...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function finalizarVenda(e) {
        ...
        const orcamentoStorageId = sessionStorage.getItem('orcamentoAtualId');
        const orcamentoStorageRaw = sessionStorage.getItem('orcamentoAtual');

        let orcamentoStorage = null;

        try {
            orcamentoStorage = orcamentoStorageRaw ? JSON.parse(orcamentoStorageRaw) : null;
        } catch (error) {
            orcamentoStorage = null;
        }
       ...
       const inputOrcamento = document.getElementById('orcamento_id');
        ...
        const orcamento_id =
            inputOrcamento?.value
                ...
                : (
                    window.orcamentoAtualId
                    || window.orcamentoAtual?.id
                    || orcamentoAtual?.id
                    || orcamentoStorageId
                    || null
                );
        console.log('ORCAMENTO_ID LIDO DO INPUT:', inputOrcamento?.value || null);
        console.log('ORCAMENTO_ID FINAL ENVIADO:', orcamento_id);
        ...
        const payloadFinalizacao = {
            ...
            itens,
            orcamento_id,
            tipo_entrega: orcamentoAtual?.tipo_entrega || null,
            cobrar_frete: orcamentoAtual?.cobrar_frete || 0,
            valor_frete: orcamentoAtual?.valor_frete || 0
        };

        console.log('PAYLOAD FINALIZAR VENDA:', payloadFinalizacao);
        ...
        try {
            ...
            console.log('DEPOIS DO CONTROLLER:', dataFinal);

            if (!res.ok || !dataFinal.success) {
                throw new Error(dataFinal.erro || 'Erro ao finalizar venda');
            }

            console.log('ANTES DE LIMPAR O PDV');

            window.carrinho = [];
            window.orcamentoAtualId = null;
            window.orcamentoAtual = null;

            sessionStorage.removeItem('orcamentoAtualId');
            sessionStorage.removeItem('orcamentoAtual');

            if (typeof PdvStorage !== 'undefined') {
                PdvStorage.carrinho = [];
                PdvStorage.limparPdv();
                PdvStorage.limparLocalStoragePDV();
            }
