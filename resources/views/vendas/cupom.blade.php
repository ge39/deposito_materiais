...
<head>
    ...
    <title>
        Cupom da Venda #{{ str_pad($venda->id, 6, '0', STR_PAD_LEFT) }}
    </title>
...
<main class="cupom">
    ...
    <header>
        <h1 class="titulo-empresa">
            {{ $empresa->nome ?? config('app.name', 'Depósito de Materiais') }}
        </h1>
    ...
    <section class="dados-venda">
        <p>
            ...
            {{ str_pad($venda->id, 6, '0', STR_PAD_LEFT) }}
        </p>
        ...
        <p>
            ...
            {{ $venda->cliente->nome ?? 'VENDA BALCÃO' }}
        </p>
    </section>
    ...
    <section>
        ...
        @forelse($venda->pagamentos as $pagamento)
            @php
            ...
            @else
                <div class="linha-valor">
                    ...
                    <span>
                        R$ {{ number_format($valorPagamento, 2, ',', '.') }}
                    </span>
                </div>
            @endif
        ...
        @empty
            @if(!empty($venda->forma_pagamento))
                <div class="linha-valor">
                    <span>
                        {{ strtoupper($venda->forma_pagamento) }}
                    </span>
        ...
        @if($trocoExibicao > 0)
            <div class="linha-valor linha-troco">
                ...
                <span>
                    R$ {{ number_format($trocoExibicao, 2, ',', '.') }}
                </span>
            </div>
        @endif
    </section>
    ...
    <section>
        <div class="linha-valor">
            ...
            <span>
                R$ {{ number_format($totalBrutoExibicao, 2, ',', '.') }}
            </span>
        </div>

        @if($descontoTotalExibicao > 0.05)
        ...
        <div class="linha-valor linha-total-liquido">
            ...
            <span>
                R$ {{ number_format($totalLiquidoExibicao, 2, ',', '.') }}
            </span>
        </div>
    </section>
