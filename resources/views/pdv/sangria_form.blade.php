...
<div class="container py-5">
    <div class="card shadow-lg border-0">
        ...
        <div class="card-header {{ (isset($bloquearPDV) && $bloquearPDV) ? 'bg-danger text-white' : 'bg-warning text-dark' }}">
            <h4 class="fw-bold mb-0">
                @if(isset($bloquearPDV) && $bloquearPDV)
                    🚫 BLOQUEIO DE CAIXA
                @else
        ...
        <div class="card-body text-center">
            ...
            <div class="bg-light p-3 rounded mb-4">
                <h5 class="mb-2">
                    ...
                    <span class="fw-bold text-dark" id="saldoAtualTexto">
                        R$ {{ number_format($saldoAtual ?? $saldoDinheiroAtual ?? 0, 2, ',', '.') }}
                    </span>
                </h5>
                <p class="text-muted mb-0">
                    Limite de Alerta Configurado: 
                    <span class="fw-bold text-danger">
                        R$ {{ number_format($limiteSangria ?? 0, 2, ',', '.') }}
                    </span>
                </p>
            </div>
            ...
            <form id="formSangria" action="{{ route('caixa.sangria.registrar', $caixa->id) }}" method="POST" class="w-50 mx-auto text-start">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <input type="number" 
                           ...
                           step="0.01" 
                           min="0.01" 
                           max="{{ $saldoAtual ?? $saldoDinheiroAtual ?? 0 }}"
                           value="{{ old('valor', number_format($saldoAtual ?? $saldoDinheiroAtual ?? 0, 2, '.', '')) }}" 
                           {{ ($saldoAtual ?? $saldoDinheiroAtual ?? 0) <= 0 ? 'disabled' : '' }} 
                           required>
                    @error('valor')
                ...
                <div class="mb-4">
                    ...
                    <select name="motivo" id="motivo" class="form-select @error('motivo') is-invalid @enderror" {{ ($saldoAtual ?? $saldoDinheiroAtual ?? 0) <= 0 ? 'disabled' : '' }} required>
                        <option value="">Selecione um motivo...</option>
                        <option value="manual" {{ old('motivo') == 'manual' ? 'selected' : '' }}>Manual / Rotina</option>
                        <option value="limite_excedido" {{ old('motivo') == 'limite_excedido' || (isset($bloquearPDV) && $bloquearPDV) ? 'selected' : '' }}>Limite do Caixa Excedido</option>
                        <option value="encerramento" {{ old('motivo') == 'encerramento' ? 'selected' : '' }}>Encerramento de Turno</option>
                    </select>
                    @error('motivo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="d-flex flex-column align-items-center gap-2 mt-4">
                    <div class="d-flex justify-content-center gap-3 w-100">
                        <button type="submit" id="btnSangria" class="btn btn-success fw-bold px-4" {{ ($saldoAtual ?? $saldoDinheiroAtual ?? 0) <= 0 ? 'disabled' : '' }}>
                            ✅ Efetuar Sangria
                        </button>
