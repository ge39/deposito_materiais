...
<body>
    ...
    <div class="ticket">
        ...
        <div class="txt-center item-linha">Código: CC-{{ str_pad($pagamento->codigo_mov, 6, '0', STR_PAD_LEFT) }}</div>
        <div class="txt-center item-linha text-uppercase">Cliente: {{ $pagamento->cliente_nome }}</div>
        <div class="txt-center item-linha">Data: {{ \Carbon\Carbon::parse($pagamento->created_at)->format('d/m/Y H:i') }}</div>
        
        <hr>
        
        <div class="txt-center item-linha"><b>SALDO CARTEIRA - DETALHES</b></div>
        <div class="txt-center item-linha">Valor Pago: R$ {{ number_format($pagamento->valor, 2, ',', '.') }}</div>
        <div class="txt-center item-linha text-uppercase">Forma Pgto: {{ $pagamento->tipo }}</div>
        <div class="txt-center item-linha text-uppercase">Via Meio: {{ $pagamento->origem ?: 'DINHEIRO' }}</div>
        <div class="txt-center item-linha">Saldo Posterior: R$ {{ number_format($pagamento->saldo_apos, 2, ',', '.') }}</div>
