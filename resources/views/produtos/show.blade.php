...
<div class="container">
    <h1>Detalhes do Produto 
        <span class="text-primary">- {{ $produto->nome }}</span>
    </h1>
    ...
    <div class="row" style="border:1px solid #ddd; padding:15px; border-radius:5px; background-color:#f9f9f9;">
        ...
        <div class="col-md-4">
            <div class="mb-3"><strong>Codigo:</strong> 000{{ $produto->id }}</div>
            <div class="mb-3"><strong>Nome:</strong> {{ $produto->nome }}</div>
            <div class="mb-3"><strong>Descrição:</strong> {{ $produto->descricao ?? '-' }}</div>
            ...
            <div class="mb-3 text-primary"><strong>Preço médio de compra:</strong> R$ {{ number_format($produto->preco_medio_compra, 2, ',', '.') }}</div>
    ...
    <a href="{{ url()->previous() }}" class="btn btn-secondary mt-4">Voltar</a>
</div>
@endsection
