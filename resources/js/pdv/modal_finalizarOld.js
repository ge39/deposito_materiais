...
document.addEventListener('DOMContentLoaded', function () {
    ...
    //         return; // 🔥 MUITO IMPORTANTE → não deixa cair no resto
    //     }
    ...
    document.addEventListener('keydown', function (e) {
     ...
     // ==========================================
    // EVENTOS DOS INPUTS (COMPORTAMENTO DA FOTO)
    // ==========================================
    inputsPagamento.forEach(input => {
        input.addEventListener('input', function () {
            ...
            // 🔒 Se o operador tentar digitar um valor maior que o saldo do cliente (ex: digitar 30,00 na Maria)
            if (forma === 'carteira') {
                ...
                // Trava o teto exatamente no saldo do cliente (20,00). 
                // A diferença vai direto para a constante visual 'Restante' da tela de forma limpa.
            ...
            // Impede que cartões, pix ou carteira aceitem valores maiores que o total da venda
            if (forma !== 'dinheiro' && valor > restanteSemEsteInput) {
                this.value = restanteSemEsteInput.toFixed(2);
            }
            
            // Recalcula o painel Restante / Troco da tela
            atualizarResumo();
        });
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (e.key === 'Enter') {
            ...
            executarFinalizacao(); // 🔥 mesma função
    ...
    function aplicarFormasPermitidas() {
        ...
        const status = (window.cliente.status_credito || '').toLowerCase().trim();

        const carteiraAtiva = status === 'ativo';

        inputsPagamento.forEach(input => {

            const forma = (input.dataset.forma || '').toLowerCase();
            ...
            if (!permitido) {
                ...
                if (forma === 'carteira') {
                    ...
                    if (status === 'bloqueado') {
                        ...
                        input.placeholder = 'Carteira inativa';
                    } 
                    else {
